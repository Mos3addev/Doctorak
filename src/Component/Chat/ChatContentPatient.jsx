...
export default function ChatContentPatient() {
    let {userData }= useContext(AuthContext)
    let {messengerPatient,MessengerRecepients}= useContext(context)
  ...
  const {id} =  useParams()
  const receiver = id;
  const receiverData = messengerPatient.find(item=> item.id === id)
        ...
        <div className="chat-header">
            ...
            <div className="media">
                ...
                <div className="media-body">
                    <div className="user-name">Dr. {receiverData.firstName} {receiverData.lastName}</div>
...
}else if(receiverData === undefined){
  return(
    <>
    <div className="chat-cont-right">
      {showPopup && (
        <div className="popup w-100">
          <div className="chat-footer w-100">
            <p>Enter Your First Message Here</p>
            <form onSubmit={sendMessage} className="input-group d-flex align-items-center">
                  <input type="text" name="message" className="input-msg-send form-control" placeholder="Type something"/>
                  <div className="input-group-append">
                      <button type="submit" className="btn msg-send-btn"><i className="fab fa-telegram-plane"></i></button>
